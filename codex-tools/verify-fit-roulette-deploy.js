...
assert(html.includes('src="./app.js?v=1.5.4"'), "Versioned app script path is not project-path relative.");
assert(html.includes('src="./context-engine.js?v=1.5.4"'), "Versioned Context Engine path is missing.");
assert(html.includes('navigator.serviceWorker.register("./sw.js?v=1.5.4", { updateViaCache: "none" })'), "Versioned early service-worker update bootstrap is missing.");
assert(html.indexOf('navigator.serviceWorker.register("./sw.js?v=1.5.4"') < html.indexOf('src="./app.js?v=1.5.4"'), "Service-worker update bootstrap must run before the app bundle.");
...
assert(html.includes('href="./styles.css?v=1.5.4"'), "Versioned stylesheet path is not project-path relative.");
...
assert(sw.includes('CACHE_NAME = "fit-roulette-v1.5.4"'), "Service-worker cache name is not synchronized to 1.5.4.");
assert(sw.includes('ASSET_VERSION = "1.5.4"'), "Service-worker install asset version is not synchronized to 1.5.4.");
...
assert(app.includes('APP_VERSION = "1.5.4"'), "Visible app version is not 1.5.4.");
assert(app.includes('STORAGE_KEY = "fitRoulette.v1"'), "localStorage key changed unexpectedly.");
assert(readme.includes("Current release: **1.5.4 - Daily Workflow Field Fixes**"), "README release metadata is not synchronized.");
