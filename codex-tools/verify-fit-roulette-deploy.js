...
assert(html.includes('src="./app.js?v=1.5.2"'), "Versioned app script path is not project-path relative.");
assert(html.includes('src="./context-engine.js?v=1.5.2"'), "Versioned Context Engine path is missing.");
assert(html.includes('navigator.serviceWorker.register("./sw.js?v=1.5.2", { updateViaCache: "none" })'), "Versioned early service-worker update bootstrap is missing.");
assert(html.indexOf('navigator.serviceWorker.register("./sw.js?v=1.5.2"') < html.indexOf('src="./app.js?v=1.5.2"'), "Service-worker update bootstrap must run before the app bundle.");
...
assert(html.includes('href="./styles.css?v=1.5.2"'), "Versioned stylesheet path is not project-path relative.");
...
assert(sw.includes('CACHE_NAME = "fit-roulette-v1.5.2"'), "Service-worker cache name is not synchronized to 1.5.2.");
assert(sw.includes('ASSET_VERSION = "1.5.2"'), "Service-worker install asset version is not synchronized to 1.5.2.");
...
assert(app.includes('APP_VERSION = "1.5.2"'), "Visible app version is not 1.5.2.");
assert(app.includes('STORAGE_KEY = "fitRoulette.v1"'), "localStorage key changed unexpectedly.");
assert(readme.includes("Current release: **1.5.2 - Narrow Editor Hotfix**"), "README release metadata is not synchronized.");
assert(changelog.includes("Fit Roulette v1.5.2 &mdash; Narrow Editor Hotfix"), "v1.5.2 changelog entry is missing.");
