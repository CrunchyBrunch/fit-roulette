...
assert(html.includes('src="./app.js?v=1.6.0"'), "Versioned app script path is not project-path relative.");
assert(html.includes('src="./context-engine.js?v=1.6.0"'), "Versioned Context Engine path is missing.");
assert(html.includes('src="./insights.js?v=1.6.0"'), "Versioned Insights path is missing.");
assert(html.includes('navigator.serviceWorker.register("./sw.js?v=1.6.0", { updateViaCache: "none" })'), "Versioned early service-worker update bootstrap is missing.");
assert(html.indexOf('navigator.serviceWorker.register("./sw.js?v=1.6.0"') < html.indexOf('src="./app.js?v=1.6.0"'), "Service-worker update bootstrap must run before the app bundle.");
...
assert(html.includes('href="./styles.css?v=1.6.0"'), "Versioned stylesheet path is not project-path relative.");
...
assert(sw.includes('CACHE_NAME = "fit-roulette-v1.6.0"'), "Service-worker cache name is not synchronized to 1.6.0.");
assert(sw.includes('ASSET_VERSION = "1.6.0"'), "Service-worker install asset version is not synchronized to 1.6.0.");
...
assert(sw.includes('"./insights.js"'), "Service worker does not cache the Insights module.");
...
assert(app.includes('APP_VERSION = "1.6.0"'), "Visible app version is not 1.6.0.");
assert(app.includes('STORAGE_KEY = "fitRoulette.v1"'), "localStorage key changed unexpectedly.");
assert(readme.includes("Current release: **1.6.0 - Closet Insights Foundation**"), "README release metadata is not synchronized.");
assert(fs.existsSync(path.join(root, "INSIGHTS.md")), "Insights contract documentation is missing.");
assert(changelog.includes("Fit Roulette v1.6.0 &mdash; Closet Insights Foundation"), "v1.6.0 changelog entry is missing.");
assert(changelog.includes("Fit Roulette v1.5.4 &mdash; Daily Workflow Field Fixes"), "v1.5.4 changelog entry is missing.");
