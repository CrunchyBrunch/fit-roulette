...
assert(html.indexOf('src="./context-engine.js?v=1.5.0"') < html.indexOf('src="./smart-closet.js?v=1.5.0"'), "Context Engine must load before Smart Closet.");
assert(html.indexOf('src="./smart-closet.js?v=1.5.0"') < html.indexOf('src="./app.js?v=1.5.0"'), "Smart Closet data module must load before the app.");
assert(html.indexOf('navigator.serviceWorker.register("./sw.js?v=1.5.0"') < html.indexOf('src="./context-engine.js?v=1.5.0"'), "Versioned service-worker update bootstrap must run before cached application bundles.");
for (const id of [
...
assert(app.includes('APP_VERSION = "1.5.0"'), "Visible release version must be 1.5.0.");
assert(sw.includes('CACHE_NAME = "fit-roulette-v1.5.0"'), "Final service-worker cache must match the 1.5.0 release.");
